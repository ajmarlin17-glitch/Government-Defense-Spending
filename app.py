...
LOGO_PATH = Path(__file__).resolve().parent / "Assets" / "TecSolutions_Logo.svg"


def _logo_svg(height_px: int = 40) -> str:
    """Load the TEC logo SVG and size it for inline display in the header."""
    try:
        svg = LOGO_PATH.read_text(encoding="utf-8")
    except OSError:
        return ""
    width_px = round(height_px * (513.58667 / 256.76001))
    svg = svg.replace('height="256.76001"', f'height="{height_px}"')
    svg = svg.replace('width="513.58667"', f'width="{width_px}"')
    return svg
...
st.markdown(
    ...
    [data-testid="stSidebar"] {{ background-color: {CARD_BG}; border-right: 2px solid {MAROON_FOOTER}; }}
    ...
    .tagline {{ color: {GREY_SECONDARY}; font-size: 0.82rem; font-style: italic; margin: 2px 0 0 0; }}
    ...
    .stTabs [data-baseweb="tab-list"] {{ gap: 4px; border-bottom: 2px solid {GREY_LIGHT}; }}
    .stTabs [data-baseweb="tab"] {{
        background-color: transparent; border-radius: 6px 6px 0 0;
        color: {GREY_SECONDARY}; padding: 10px 22px; font-weight: 700;
    }}
    .stTabs [aria-selected="true"] {{ color: {MAROON_TITLE} !important; border-bottom: 3px solid {MAROON_FOOTER} !important; }}
...
# --- Header: TEC logo, eyebrow, title, subtitle, brand tagline ---
logo_markup = _logo_svg(40)
if logo_markup:
    st.markdown(f'<div style="margin-bottom:8px;">{logo_markup}</div>', unsafe_allow_html=True)
...
st.markdown('<p class="tagline">TEC: Where the people matter</p>', unsafe_allow_html=True)
        ...
        with col_type:
            with st.container(border=True):
                ...
                fig_type = go.Figure(go.Bar(
                    ...
                    orientation="h", marker_color=MAROON_FOOTER,
                    text=[core.money(v) for _, v in type_items], textposition="outside",
                ))
                fig_type.update_layout(
                    **CHART_LAYOUT_DEFAULTS,
                    height=340, margin=dict(l=10, r=10, t=10, b=10),
                    xaxis=dict(title="USD billions", gridcolor=GREY_LIGHT),
        ...
        with col_branch:
            with st.container(border=True):
                ...
                fig_branch = go.Figure(go.Bar(
                    ...
                    orientation="h", marker_color=GREY_SECONDARY,
