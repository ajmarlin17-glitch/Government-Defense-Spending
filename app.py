...
if active_view == "Spending categories":
    ...
    if category_error:
        ...
        with st.container(border=True):
            ...
            st.dataframe(
                pd.DataFrame([{"Account": a["name"], "Appropriation type": a["type"], "Branch": a["branch"], "Amount": core.money_precise(a["outlay_billions"])} for a in sorted(accounts, key=lambda a: -a["outlay_billions"])]),
    ...
    with st.container(border=True):
        ...
        if naics_error:
            ...
            st.dataframe(
                pd.DataFrame([{"NAICS category": r["name"], "Amount": core.money_precise(r["amount"])} for r in naics_results]),
                use_container_width=True, hide_index=True,
            )
...
if active_view == "TEC relevance":
    ...
    if not category_error and not naics_error and tec["tec_naics_items"]:
        ...
        with st.container(border=True):
            ...
            st.dataframe(
                pd.DataFrame([
                    {"NAICS category": item["name"], "Code": item.get("code") or "—", "Amount": core.money_precise(item["amount"])}
