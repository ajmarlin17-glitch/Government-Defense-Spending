...
This module contains no UI code (no Streamlit, no Tkinter) so it can be
imported by both the web app (app.py) and unit tests. It is a direct port of
the analysis engine from the desktop app (V7), with two Windows-specific
pieces removed since this runs in a Linux-hosted web environment:

- The PowerShell download fallback (Invoke-WebRequest) is gone; a plain
  urllib request with a bundled CSV snapshot fallback is used instead.
- Local filesystem paths assume this file sits next to Data/ and Config/
  folders in the same deployment, rather than a PyInstaller-frozen .exe.
"""
...
def load_congress_api_key(streamlit_secrets: dict | None = None) -> str | None:
    """Look for the Congress.gov API key in Streamlit secrets first (the
    correct place to store it for a deployed web app, since anything in a
    Config file would be visible to anyone who can see the source), falling
    back to a local Config file for local development only."""
...
# Treasury's API sits behind a firewall/WAF that appears to reset the
# connection ("RemoteDisconnected") for requests that don't look like they
# came from a real browser. A full, realistic header set plus a couple of
# retries clears this up in most hosting environments; the urllib fallback
# below is a second attempt in case the `requests` library itself is
# unavailable or blocked in a given environment.
_BROWSER_HEADERS = {
    "User-Agent": (
...
def _fetch_via_requests(timeout: int, attempts: int = 3):
    import requests
    ...
    response = session.get(DATA_URL, headers=_BROWSER_HEADERS, timeout=timeout)
    response.raise_for_status()
    return response.text


def _fetch_via_urllib(timeout: int, attempts: int = 3):
    import time

    last_exc = None
    for attempt in range(attempts):
        try:
            request = urllib.request.Request(DATA_URL, headers=_BROWSER_HEADERS)
            with urllib.request.urlopen(request, timeout=timeout) as response:
...
def fetch_data(timeout: int = 30):
    """Fetch live Treasury data, falling back to the bundled snapshot CSV if
    the network request fails or is blocked (e.g. by a restrictive
    corporate/hosting firewall). Tries the `requests` library first (better
    retry/backoff and connection handling), then falls back to a retrying
    urllib attempt, before giving up and using the snapshot."""
    errors = []

    try:
        text = _fetch_via_requests(timeout)
    ...
    try:
        text = _fetch_via_urllib(timeout)
        rows, current, monthly = parse_treasury_json(text)
        return rows, current, monthly, f"Live Treasury data through {current[2]}"
    except Exception as exc:
    ...
    return rows, current, monthly, f"Verified Treasury snapshot through {current[2]} (live fetch unavailable: {'/'.join(errors)})"
...
def _one_step_mape(values, transformed, damping=None):
    ...
    for stop in range(5, len(values)):
        ...
        probe = ModelResult(
            name="", intercept=intercept, slope=slope, transformed=transformed,
            backtest_mape=0.0, residual_std=residual_std, x_mean=x_mean, sxx=sxx,
            count=stop, damping=damping,
        )
...
def build_candidates(values):
    candidates = []
    for transformed, name, damping in (
        (False, "Linear trend", None),
        (True, "Log-linear growth trend", None),
        (True, "Damped log-linear trend", DAMPING_PHI),
    ):
        intercept, slope, residual_std, x_mean, sxx = _ols(values, transformed)
        candidates.append(
            ModelResult(
                name=name, intercept=intercept, slope=slope, transformed=transformed,
                backtest_mape=_one_step_mape(values, transformed, damping),
                residual_std=residual_std, x_mean=x_mean, sxx=sxx,
                count=len(values), damping=damping,
            )
        )
    return candidates
...
def monte_carlo_bands(last_value, notes, growth_std, trials=MC_TRIALS, seed=MC_SEED):
    ...
    for _ in range(trials):
        ...
        for i, note in enumerate(notes):
            ...
            policy_noise = (
                rng.gauss(0.0, 0.008 + 0.02 * (1.0 - note.policy_weight))
                if note.policy_weight > 0
                else 0.0
            )
            sampled_growth = (
                note.blended_growth
                + stat_noise * (1 - note.policy_weight)
                + policy_noise * note.policy_weight
            )
...
def analyze(rows, forecast_years=FORECAST_YEARS, policy_data=None, scenario="baseline", mc_trials=MC_TRIALS, mc_seed=MC_SEED):
    ...
    model_info = ModelInfo(
        best_name=best.name,
        best_mape=best.backtest_mape,
        best_damped=best.damping is not None,
        window_years=window,
        ensemble_mape=ensemble_mape,
        weights=tuple((c.name, c.backtest_mape, w) for c, w in zip(candidates, weights)),
    )
    ...
    for step in range(1, forecast_years + 1):
        ...
        notes.append(
            PolicyNote(
                year=year,
                statistical_growth=stat_growth,
                policy_growth=p_growth,
...
def format_model_panel(model_info: ModelInfo) -> str:
    ...
    lines.append(
        "\nThe shaded area is a Monte Carlo-based approximate 95% range that "
        "incorporates statistical, congressional/CBO, and scenario uncertainty."
    )
    return "\n".join(lines)
...
def format_policy_panel(policy_notes, bill_updates, policy_data, scenario) -> str:
    ...
    if not policy_notes:
        ...
        for note in policy_notes:
            if note.policy_weight > 0:
                lines.append(
                    f"- **FY{note.year}:** statistical {note.statistical_growth * 100:+.1f}%/yr, "
                    f"policy {note.policy_growth * 100:+.1f}%/yr "
                    f"({note.policy_label}, weight {note.policy_weight * 100:.0f}%) "
                    f"→ blended {note.blended_growth * 100:+.1f}%/yr"
                )
            else:
                lines.append(
                    f"- **FY{note.year}:** statistical {note.statistical_growth * 100:+.1f}%/yr "
                    f"(no enacted/proposed/CBO figure yet) → blended {note.blended_growth * 100:+.1f}%/yr"
                )
    scenario_adjustment = policy_data.get("scenarios", {}).get(scenario, 0.0)
