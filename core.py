...
USASPENDING_FEDERAL_ACCOUNT_URL = (
    ...
    "?fiscal_year={fiscal_year}&limit=250"
)
...
def fetch_federal_account_breakdown(fiscal_year: int, toptier_code: str = DOD_TOPTIER_CODE, timeout: int = 30):
    url = USASPENDING_FEDERAL_ACCOUNT_URL.format(toptier_code=toptier_code, fiscal_year=fiscal_year)
...
def fetch_naics_breakdown(fiscal_year: int, limit: int = 15, timeout: int = 30):
    import requests
    from requests.adapters import HTTPAdapter
    from urllib3.util.retry import Retry
    ...
    return [
        {"name": item.get("name") or item.get("code", "Unknown"), "amount": float(item.get("amount") or 0.0) / 1_000_000_000}
        for item in results
    ]
