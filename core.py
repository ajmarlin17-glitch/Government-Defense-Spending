...
def money_precise(value: float) -> str:
    """Format a USD-billions value for a table cell, switching to millions
    for small amounts. A raw ``round(value, 2)`` on a $2M line item (0.002B)
    displays as a bare "0", making tiny federal accounts look like missing
    data rather than small dollar figures -- this shows "$2.0M" instead."""
    if value == 0:
        return "$0.0M"
    if abs(value) < 0.01:
